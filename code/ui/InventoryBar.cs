...
public class InventoryBar : Panel
{
	...
	public override void Tick()
	{
		...
		{
			UpdateIcon( player.Inventory.GetSlot( i ), slots[i], i, slots.Count );
			if ( player.Inventory.GetSlot( i ) != null )
			{
				finalIndex = i;
			}
		}

		UpdateIcon(player.Inventory.GetSlot( finalIndex ), slots[finalIndex], finalIndex, finalIndex );
	}

	private static void UpdateIcon( Entity ent, InventoryIcon inventoryIcon, int i, int max )
	{
		if ( ent == null )
		{
			inventoryIcon.Clear();
			inventoryIcon.SetClass( "empty", true );
			return;
		}

		inventoryIcon.SetClass( "empty", false );

		inventoryIcon.SetClass( "first", i == 0 );
		inventoryIcon.SetClass( "last", i == max );
